...
int main(void){
       ...
       SysCtlClockSet(SYSCTL_SYSDIV_2_5 | SYSCTL_USE_PLL | SYSCTL_XTAL_16MHZ | SYSCTL_OSC_MAIN);
        ...
        initI2C();//calls the initI2C function
        initMPU6050();//calls the initMPU function
        MPUtestConnection();//calls the MPU test function




        while(1){
            readMPU();//calls the readMPU function
            SysCtlDelay(1000000);
            getAngle(Acc[0],Acc[1],Acc[2]);//calls the getAngle function
            ComplementaryFilter();//calls the filter function
            UARTprintf("Pitch :%d\n",p);//print the pitch
            UARTprintf("Roll :%d\n", r);//print the roll
            UARTprintf("Yaw :%d\n", y);//print the yaw
...
void MPUtestConnection()//check the MPU6050  works
{

    readI2C(MPU_ADDRESS, WHO_AM_I, &WhoAmI);//read the who am I
    if(WhoAmI == 0x68)//check if the who am I is 0x68
    {
        UARTprintf("Connection successful ! \n");//if is print message
        GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, 4);//turn on the green led
        SysCtlDelay(1000000);
        GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, 0x0);//turn off the green led
    }

}

void readMPU()//read the Gyro and Accel values from the chip
{



    //below reads the Accel values
    ...
    //store the high and low value of the Accel into the AccX,AccY,AccZ variable
    accXout = ((accXout_H << 8) | accXout_L);
    accYout = ((accYout_H << 8) | accYout_L);
    accZout = ((accZout_H << 8) | accZout_L);

    //store the high and low value of the Gyro into the gyroX,gyroY,gyroZ variable
    ...
    //store the AccX,AccY,AccZ into the iq16 array
    Acc[0] =  _IQ16(accXout);
    Acc[1] =  _IQ16(accYout);
    Acc[2] =  _IQ16(accZout);

    //store the gyroX,gyroY,gyroZ into the iq16 array
